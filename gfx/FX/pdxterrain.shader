...
PixelShader =
{
	...
	MainCode PixelShader
	{
		...
		[[
			...
			float3 GameCalculateSecondSunLighting( SMaterialProperties MaterialProps, SLightingProperties LightingProps, float3 WorldSpacePos, inout float IridescenceMask  )
			{
				float3 SpecularLight = vec3( 0.0f );
				...
				// Sun specular light
				float3 F = F_Schlick( MaterialProps._SpecularColor, vec3( 1.0f ), LdotH );
				float D = D_GGX( NdotH, lerp( 0.03f , 0.33f , MaterialProps._Roughness ) );		// Remap to avoid super small and super bright highlights
				#ifdef PDX_SimpleLighting
					float Vis = V_Optimized( LdotH, MaterialProps._Roughness );
				#else
					float Vis = V_Schlick( NdotL, NdotV, MaterialProps._Roughness );
			...
			PDX_MAIN
			{
				...
				// second sun test (should use customised terrain specific calculate lighting method?
				SLightingProperties SecondLightingProps = GetSecondSunLightingProperties( Input.WorldSpacePos );
				FinalColor += GameCalculateSecondSunLighting( MaterialProps, SecondLightingProps, Input.WorldSpacePos, IridescenceMask );
